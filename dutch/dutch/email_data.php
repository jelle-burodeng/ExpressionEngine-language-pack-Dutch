...
if ( ! function_exists('admin_notify_mailinglist_title'))
{
	...
	{
return <<<EOF
Iemand heeft zich ingeschreven voor je mailinglijst.
EOF;
	}
}
...
Om je account te activeren, klik op de volgende link:
...
Om je wachtwoord te wijzigen, ga naar de volgende pagina:

{reset_url}

Uw wachtwoord zal automatisch gereset worden en een nieuw wachtwoord ontvang je per e-mail.

Indien je niet je wachtwoord wilt wijzigen, negeer deze e-mail dan. Na 24 uur zal deze link niet meer actief zijn.
...
Hier is je nieuwe login informatie:
...
Sorry, maar onze staf heeft bepaald je account niet te activeren.
...
Als je niet lid wilt worden, negeer deze e-mail dan.
...
if ( ! function_exists('comment_notification_title'))
{
	...
	{
return <<<EOF
Iemand heeft gereageerd op je bericht
EOF;
	}
}
...
if ( ! function_exists('comment_notification'))
{
	...
	{
return <<<EOF
{name_of_commenter} heeft gereageerd op een bericht waar je voor ingeschreven bent op:
{channel_name}
...
if ( ! function_exists('comments_opened_notification'))
{
	...
	{
return <<<EOF
Nieuwe reacties zijn toegevoegd aan het bericht waar je voor ingeschreven bent op:
{channel_name}
...
if ( ! function_exists('admin_notify_gallery_comment'))
{
	...
	{
return <<<EOF
Je hebt een reactie ontvangen op de volgende fotogallery:
{gallery_name}
...
if ( ! function_exists('gallery_comment_notification_title'))
{
	...
	{
return <<<EOF
Iemand heeft gereageerd op je reactie
EOF;
	}
}
...
if ( ! function_exists('gallery_comment_notification'))
{
	...
	{
return <<<EOF
Iemand heeft gereageerd op de foto waar je op geabonneerd bent:
{gallery_name}
...
if ( ! function_exists('forum_post_notification'))
{
	...
	{
return <<<EOF
Iemand heeft een post gedaan in een thread waarop je geabonneerd bent op:
{forum_name}
...
if ( ! function_exists('private_message_notification_title'))
{
	...
	{
return <<<EOF
Iemand heeft je een prive bericht gestuurd.
EOF;
	}
}
...
{sender_name} heeft je een prive bericht gestuurd met de titel ‘{message_subject}’.

Je kunt je privebericht zien door in te loggen op je inbox via:
{site_url}
...
Om verdere notificaties over prive berichten te stoppen, zet deze functie dan uit in je e-mail instellingen.
...
{sender_name} wilde je zojuist een prive bericht sturen,
maar je inbox is vol. Het overschrijft het maximum van {pm_storage_limit} berichten.

Log in en verwijder overtollige berichten uit je inbox op:
{site_url}
EOF;
	}
}
...
Om je uit te schrijven van deze e-mai,l klik hier:
